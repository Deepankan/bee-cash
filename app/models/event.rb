class Event < ApplicationRecord
	...
	def event_created_by
		User.find_by_id(self.created_by).try(:name)
	end

	def event_valid
		Event.event_valid(self.start_date)
	end

	def attend(user)
		(user.present? ? (self.users.include?(user) == true ?  ENABLE : DISABLE): DISABLE)
	end

	def event_start_date
		Event.get_date_format(self.start_date)
	end

	def event_end_date
		Event.get_date_format(self.end_date)
	end
